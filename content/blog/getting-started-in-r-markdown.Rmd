...
Code chunks in R Markdown documents are used to write R code. Every time you want to include R code, you will need to enclose it with three backwards apostrophes. For instance, to compute the mean of the values 1, 7 and 11, we first need to insert a R chode chunk by clicking on the `Insert` button located at the top and select R (see below a picture), then we need to write the corresponding code inside the code chunk we just inserted:
...
* [Link](https://www.statsandr.com/): `[link](https://www.statsandr.com/)` (do not forget `https://` if it is an external URL)
* Equations:
...
To continue learning about R Markdown, see two complete cheat sheets from the R Studio team [here](https://rstudio.com/wp-content/uploads/2015/02/rmarkdown-cheatsheet.pdf) and [here](https://rstudio.com/wp-content/uploads/2015/03/rmarkdown-reference.pdf), and a more complete guide [here](https://bookdown.org/yihui/rmarkdown/) written by Yihui Xie, J. J. Allaire and Garrett Grolemund.
