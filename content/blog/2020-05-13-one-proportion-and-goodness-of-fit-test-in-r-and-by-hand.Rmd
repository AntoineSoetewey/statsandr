...
The test statistic and degrees of freedom are exactly the same than the ones found by hand. The *p*-value is `r round(test$p.value, 3)` which, still at the 5% significance level, leads to the same conclusion than by hand (i.e., failing to reject the null hypothesis). Here again you can use the [Shiny app](/blog/a-guide-on-how-to-read-statistical-tables/) to easily compute the *p*-value given the test statistic.
