...
The test statistic and degrees of freedom are exactly the same than the ones found by hand. The *p*-value is `r round(test$p.value, 3)` which, still at the 5% significance level, leads to the same conclusion than by hand (i.e., fail to reject the null hypothesis).

Thanks for reading. I hope this article helped you understand and perform the one-proportion and goodgness of fit test in R and by hand.
