...
As you can imagine, the solution in R us much shorter and requires much less computation on the user side. We first need to enter the data as a matrix:
...
Step 2. From the distance matrix computed in step 1, we see that the **smallest distance** = 0.328 between points 2 and 4. Since points 2 and 4 are the closest to each other, these 2 points are put together to form a single group. The new distances between the points 2 and 4 and all other points are now:



Step 3. The new coordinates 

Solution in R :

```{r}
X <- matrix(c(2.03,0.06,-0.64,-0.10,-0.42,-0.53,-0.36,0.07,1.14,0.37), nrow = 5, byrow = TRUE)

#Algorithme de Ward et dendrogramme
hclust_ward <- hclust(dist(X)^2 / (2 * nrow(X)), method = "ward.D")
plot(hclust_ward)
