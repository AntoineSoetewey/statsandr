...
p <- ggplot(dr, aes(x = X, y = Y, na.rm = T)) +
  ...
  stat_summary(fun = "mean", fun.min = "mean", fun.max = "mean", size = 0.3, width = 0.33, geom = "crossbar") +
  ylab("Height (in cm)") +
  xlab("")
plot(p)
```
...
The standard deviation for this sample was `r round(sd(tail(height, 3)), 2)` cm, so the variance of the height of these adults is `r round(var(tail(height, 3)), 2)` $cm^2$.
