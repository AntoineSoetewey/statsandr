...
Final remark regarding the interpretation of a $Z$ score: a rule of thumb is that a value with $Z$ score between -3 and -2 or between 2 and 3 is considered as a rare value. A value with $Z$ score smaller than -3 or larger than 3 is considered as an extremely rare value. A value with any other $Z$ score is considered as not rare nor extremely rare. 
...
In this section, we show 3 complementary methods to determine whether your data follow a normal distribution in R.
...
Density plots also provides a visual judgment about whether the data follow a normal distribution. They are similar to histograms as they also allows to analyze the spread of the distribution. However, they are a smoothed version of the histogram. Here is the density plot drawn from the dataset on the height of 12 adults discussed above:
...
Like histograms and density plots, QQ-plots allow to visually evaluate the normality assumption. Here is the QQ-plot drawn from the dataset on the height of 12 adults discussed above:
...
```{r, echo = FALSE}
ggqqplot(rbeta(100,6, 1))
```

Note that QQ-plots are also a convenient way to assess whether residuals from regression analysis follow a normal distributed.
...
* the alternative hypothesis is that the data do not follow a normal distribution

Shapiro-Wilk test is recommended for normality test as it provides better power than Kolmogorov-Smirnov test.^[The Shapiro-Wilk test is based on the correlation between the sample and the corresponding normal scores.] In R, the Shapiro-Wilk test of normality can be done with the function `shapiro.test()`:
...
It is important to note that, in practice, normality tests are often considered as too conservative in the sense that for large sample size ($n > 50$), a small deviation from the normality may cause the normality condition to be violated. A normality test is a hypothesis test, so as the sample size increases, their capacity of detecting smaller differences increases. So as the number of observations increases, the Shapiro-Wilk test becomes very sensitive even to a small deviation from normality. As a consequence, it happens that accordint to the normality test the data do not follow a normal distribution although the departures from the normal distribution are negligible and the data in fact follow a normal distribution. For this reason, it is often the case that the normality condition is verified based on a combination of all methods presented in this article, that is, visual inspections with histograms and QQ-plots and a formal inspection with the Shapiro-Wilk test for instance.

I personnaly tend to prefer QQ-plot over histograms and normality tests so I do not have to mind about the sample size. This article showed the different methods that are available, your choice will of course depends on the type of your data and the context of your analyses.

Thanks for reading. I hope the article helped you to learn more about the normal distribution and how to test for normality in R. See other articles about [statistics](/tags/statistics) or about [R](/tags/R/).
