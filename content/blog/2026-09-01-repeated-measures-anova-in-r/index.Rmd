...
If, even after a transformation of the data, normality was clearly not satisfied, the nonparametric alternative to the repeated measures ANOVA should be used: the [Friedman test](/blog/friedman-test-nonparametric-version-repeated-measures-anova/), which compares the conditions based on ranks instead of means and which requires neither normality nor sphericity.
...
* If sphericity is severely violated, alternatives to the corrections exist: a multivariate approach (MANOVA), a linear mixed model, or the nonparametric [Friedman test](/blog/friedman-test-nonparametric-version-repeated-measures-anova/). The MANOVA approach is attractive here because it works directly with the covariance structure of the difference variables and makes no sphericity assumption at all, so it sidesteps the problem entirely rather than correcting for it.
...
Note that if the normality assumption had not been satisfied, the post-hoc tests would have been pairwise Wilcoxon signed-rank tests (`pairwise_wilcox_test()` with `paired = TRUE`) instead of paired t-tests, following a [Friedman test](/blog/friedman-test-nonparametric-version-repeated-measures-anova/) instead of a repeated measures ANOVA.
