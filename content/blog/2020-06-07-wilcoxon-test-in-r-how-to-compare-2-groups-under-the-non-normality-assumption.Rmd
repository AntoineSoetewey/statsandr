...
1. The **Mann-Whitney-Wilcoxon** test (also referred as Wilcoxon rank sum test or Mann-Whitney U test) is performed when the samples are **independent** (so this test is the non-parametric equivalent to the Student's t-test for independent samples).
...
We add the `paired = TRUE` argument to the `wilcox.test()` function to take into consideration the dependency between the 2 samples:

```{r}
before <- dat2$Grade[dat2$Time == "Before"]
