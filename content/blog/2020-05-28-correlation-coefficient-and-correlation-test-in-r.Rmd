...
A correlation is usually computed between two [quantitative](/blog/variable-types-and-examples/#quantitative) variables. In this article, I show how to compute correlation coefficients, and how to perform correlation tests in R and how to visualize relationships between variables.

See the [Chi-square test of independence](/blog/chi-square-test-of-independence-in-r/) if you need to study the relationship between two [qualitative](/blog/variable-types-and-examples/#qualitative) variables.
...
Note 1: the correlation between variables *x* and *y* is equal to the correlation between variables *y* and *x* so the order of the variables in the `cor()` function does not matter.

Note 2: the Pearson correlation is computed by default with the `cor()` function. If you want to compute the Spearman correlation for instance, add the argument `method = "spearman"`:
...
Run `?cor` for more information about the different methods available in the `cor()` function.
...
```{r}
# correlation for all variables (rounded to 2 decimals)
round(cor(dat),
      digits = 2)
```
...
First of all, a correlation ranges from **-1 to 1**.
...
As an illustration, the Pearson correlation between horsepower (`hp`) and miles per gallon (`mpg`) found above is `r round(cor(dat$hp, dat$mpg), 2)`, meaning that the 2 variables vary in opposite direction. This makes sense, cars with more horsepower tend to consume more fuel (and thus have a lower millage par gallon). On the contrary, from the correlation matrix we see that the correlation between miles per gallon (`mpg`) and the 1/4 mile time (`qsec`) is `r round(cor(dat$mpg, dat$qsec), 2)`, meaning that fast cars (low `qsec`) tend to have a worse millage per gallon (low `mpg`). This again make sense as fast cars tend to consume more fuel.
...
## One scatterplot for 2 variables
...
(You can use the [esquisse addin](/blog/rstudio-addins-or-how-to-make-your-coding-life-easier/#esquisse) if you want to easily draw plots with the `{ggplot2}` package.)
...
The figure indicates that weight (`wt`) and horsepower (`hp`) are positively correlated, while miles per gallon (`mpg`) is negatively correlated with horsepower (`hp`) and weight (`wt`).

## Another simple correlation matrix

This version of the correlation matrix presents the correlation coefficients in a slightly more readable way by coloring to coefficients depending on their sign. Applied to our dataset, we have:
...
Unlike a correlation matrix which indicates correlation coefficients between variables, the correlation test is used to test whether the correlation (denoted $\rho$) between 2 variables is significantly different from 0 or not.

In fact, a correlation coefficient different from 0 does not mean that the correlation is **significantly** different from 0. The hypotheses for the correlation test are as follows:

* $H_0$: $\rho = 0$
* $H_1$: $\rho \ne 0$

Suppose that we want to test whether `drat` is correlated with `qsec`:
...
This test proves that even if the correlation coefficient is slightly different from 0 (the correlation is `r round(test$estimate, 2)`), it is actually not significantly different from 0.

Note that the *p*-value of a correlation test is based on the correlation coefficient **and** the sample size. The larger the sample size and the more extreme the correlation (closer to -1 or 1), the more likely the null hypothesis of no correlation will be rejected. With a small sample size, it is thus possible to obtain a relatively large correlation, but still find a correlation not significantly different from 0. For this reason, it is recommend to always perform a correlation test before interpreting a correlation coefficient.

## For several pairs of variables

Similar to the correlation matrix used to compute correlation for several pairs of variables, the `rcorr()` function (from the `{Hmisc}` package) allows to compute the *p*-values of the correlation test for several pairs of variables at once. Applied to our dataset, we have:
...
The below figure, known as a [correlogram](/blog/correlogram-in-r-how-to-highlight-the-most-correlated-variables-in-a-dataset/), does precisely this:
...
The above plot shows correlation coefficients for all pairs of variables (with more intense colors for more extreme correlations), and correlations not significantly different from 0 are represented by white box.
