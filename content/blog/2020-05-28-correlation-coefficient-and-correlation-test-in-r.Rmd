...
As an illustration, the Pearson correlation between horsepower (`hp`) and miles per gallon (`mpg`) found above is `r round(cor(dat$hp, dat$mpg), 2)`, meaning that the 2 variables vary in opposite direction. This makes sense, cars with more horsepower tend to consume more fuel (and thus have a lower mileage per gallon). On the contrary, from the correlation matrix we see that the correlation between miles per gallon (`mpg`) and the time to drive 1/4 of a mile (`qsec`) is `r round(cor(dat$mpg, dat$qsec), 2)`, meaning that fast cars (low `qsec`) tend to have a worse mileage per gallon (low `mpg`). This again makes sense as fast cars tend to consume more fuel.
...
- X causes Y
- Y causes X
- a third variable causes X and Y
