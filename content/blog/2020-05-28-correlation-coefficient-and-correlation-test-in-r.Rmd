...
p1 <- ggplot(dat_outlier) +
  ...
  theme_minimal() +
  stat_cor(aes(label = ..r.label..), label.x = 0.5) +
  labs(
...
Note that, for small sample sizes (usually $n < 30$), the two variables should follow a [normal distribution](/blog/do-my-data-follow-a-normal-distribution-a-note-on-the-most-widely-used-distribution-and-how-to-test-for-normality-in-r/) if you want to do a correlation test or estimate the confidence interval.
...
The plot above combines correlation coefficients, correlation tests (via the asterisks next to the coefficients^[3 asterisks means that the coefficient is significant at the 5% level, 2 is at the 1% significance level, and 3 is at the 0.1% significance level.]) and scatterplots for all possible pairs of variables present in a dataset.
