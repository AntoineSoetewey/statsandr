...
Find more about this package on <a href="https://cran.r-project.org/web/packages/pacman/index.html" target="_blank" rel="noopener">CRAN</a>.

## `{librarian}` package

Like pacman, the `shelf()` function from `{librarian}` package automatically installs, updates, and load R packages that are not yet installed in a single function. The function accepts packages from CRAN, GitHub, and Bioconductor and it accepts multiple package entries, provided as a comma-separated list of unquoted names (so no `""` around package names).
...
For CRAN packages, provide the package name as normal without `""` and for GitHub packages, provide the username and package name separated by `/` (i.e., `UserName/RepoName`).

Find more about this package on <a href="https://cran.r-project.org/web/packages/librarian/index.html" target="_blank" rel="noopener">CRAN</a>.
