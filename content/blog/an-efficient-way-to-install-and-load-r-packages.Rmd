...
Like pacman, the `shelf()` function from the `{librarian}` package automatically installs, updates, and loads R packages that are not yet installed in a single function. The function accepts packages from CRAN, GitHub, and Bioconductor (only if Bioconductor's `Biobase` package is installed) and it accepts multiple package entries, provided as a comma-separated list of unquoted names (so no `""` around package names).
