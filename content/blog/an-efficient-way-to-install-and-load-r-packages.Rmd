...
Unlike other programs, only fundamental functionalities come by default with R. You will thus often need to install some "extensions" to perform the analyses you want. These extensions which are are collections of functions and datasets developed and published by R users are called **packages**. They extend existing base R functionalities by adding new ones. R is open source so everyone can write code and publish it as a package, and everyone can install a package and start using the functions or datasets built inside the package for free.

In order to use a package, it needs to be installed by running `install.packages("name_of_package")` (do not forget `""` around the name of the package!). Once the package is installed, you must load the package and only after it has been loaded you can use all the functions and datasets it contains. To load a package, run `library(name_of_package)` (this time `""` around the name of the package are optional, but can still be used if you wish).
...
Then one day, a colleague of mine shared some of his code with me. I am glad he did as I just discovered a much more efficient way to install and load R packages. He gave me the permission to share this tip, so here is the code I now use to perform the task of installing and loading R packages:
...
1. The function `install.packages()` accepts a vector as argument, so one line of code for each package is now one line including all packages
...
Thanks for reading. I hope the article helped you to install and load R packages in a more efficient way. See other articles about [R](/tags/R/).
