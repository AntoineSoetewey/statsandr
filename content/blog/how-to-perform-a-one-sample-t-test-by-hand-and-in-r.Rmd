...
Note that the data are artificial and do not represent any real variable. Furthermore, remind that the assumptions may or may not be met. The point of the article is to detail how to compute the different versions of the test by hand and in R, so all assumptions are assumed to be met. Moreover, we assume that for all tests the significance level, that is, the type I error is $\alpha = 5$% (see more examples of type I and type II error in the context of [hypothesis testing](https://365datascience.com/false-positive-vs-false-negative/){target="_blank"}).
...
* The dependent variable (i.e., the measured variable) must be measured on a [continuous](/blog/variable-types-and-examples/#continuous) or [ordinal scale](/blog/variable-types-and-examples/#ordinal).
* Normality:
