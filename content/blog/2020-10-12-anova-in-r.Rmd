...
ANOVA (ANalysis Of VAriance) is a [statistical test](/blog/what-statistical-test-should-i-do/) to determine whether two or more population means are different. In other words, it is used to **compare two or more groups** to see if they are significantly **different**.
...
In this sense, if the null hypothesis is rejected, it means that at least one species is different from the other 2, but not necessarily that all 3 species are different from each other. It could be that flipper length for the species Gentoo is different than for the species Chinstrap and Adelie, but flipper length is similar between Chinstrap and Adelie. Other types of test (known as post-hoc tests and covered in this [section](/blog/anova-in-r/#post-hoc-test)) must be performed to test whether all 3 species differ.
...
Since the smallest sample size per group (i.e., per species) is `r min(table(dat$species))`, we have large samples. Therefore, we do not need to check normality.
...
Some researchers stop here and assume that normality is met, while others also test the assumption via a formal [normality test](/blog/do-my-data-follow-a-normal-distribution-a-note-on-the-most-widely-used-distribution-and-how-to-test-for-normality-in-r/#normality-test). It is your choice to test it (i) only visually, (ii) only via a normality test, or (iii) both visually AND via a normality test. Bear in mind, however, the two following points:
...
Remember that if the normality assumption was not reached, some transformation(s) would need to be applied on the raw data in the hope that residuals would better fit a normal distribution, or you would need to use the non-parametric version of the ANOVA---the [Kruskal-Wallis test](/blog/kruskal-wallis-test-nonparametric-version-anova/).
...
There are a couple of [outliers](/blog/outliers-detection-in-r/) as shown by the points outside the whiskers, but this does not change the fact that the [dispersion](/blog/descriptive-statistics-by-hand/#dispersion) is more or less the same between the different species.
...
Besides a boxplot for each species, it is also a good practice to compute some [**descriptive statistics**](/blog/descriptive-statistics-in-r/) such as the [mean](/blog/descriptive-statistics-in-r/#mean) and [standard deviation](/blog/descriptive-statistics-in-r/#standard-deviation-and-variance) by species.
...
However, if several t-tests are performed, the issue of **multiple testing** (also referred as multiplicity) arises. In short, when several [statistical tests](/blog/what-statistical-test-should-i-do/) are performed, some will have *p*-values less than $\alpha$ purely by chance, even if all null hypotheses are in fact true.
...
The first one is edited by me based on the code found in this [article](http://www.sthda.com/english/articles/24-ggpubr-publication-ready-plots/76-add-p-values-and-significance-levels-to-ggplots/){target="_blank"}:
...
Thanks for reading. See this [tutorial](/blog/how-to-one-way-anova-by-hand/) if you would like to learn how to do an ANOVA by hand.
