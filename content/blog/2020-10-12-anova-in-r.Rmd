...
As pointed out by a reader (see comments at the very end of the article), the normality assumption can also be tested on the "raw" data instead of the residuals. However, if you test the normality assumption on the raw data, it must be tested for *each group separately*.

Indeed, saying "The distribution of Y within each group is normally distributed" is the same as saying "The residuals are normally distributed".
...
1. Checking normality on the residuals (but not per group)
...
If you are still not convinced: remember that an ANOVA is a special model of a linear model. Suppose your independent variable is a [continuous variable](/blog/variable-types-and-examples/#continuous) (instead of a [categorical variable](/blog/variable-types-and-examples/#qualitative)), the only option is to check normality on the residuals.
...
If you are interested in including results of ANOVA and post-hoc tests on the same plot (directly on the boxplots), here is a piece of code which may be of interest to you (edited by myself based on the code found in this [article](http://www.sthda.com/english/articles/24-ggpubr-publication-ready-plots/76-add-p-values-and-significance-levels-to-ggplots/){target="_blank"}):
