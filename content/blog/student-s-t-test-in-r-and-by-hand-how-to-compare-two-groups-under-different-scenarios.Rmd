...
3. Critical value: $-t_{\alpha, \upsilon}$ where $$\upsilon = \frac{\bigg(\frac{s^2_1}{n_1} + \frac{s^2_2}{n_2} \bigg)^2}{\frac{\bigg(\frac{s^2_1}{n_1}\bigg)^2}{n_1 - 1} + \frac{\bigg(\frac{s^2_2}{n_2}\bigg)^2}{n_2 - 1}} = 8.28$$ so $$-t_{0.05, 8.28} = -1.851$$
*Note:* The degrees of freedom 8.28 does not exist in the standard Student distribution table, so simply take 8, or compute it in R with `qt(p = 0.05, df = 8.28)`. For simplicity, this number of degrees of freedom is sometimes approximated as $df = min(n_1 - 1, n_2 - 1)$, so in this case it would be $df = 4$.
