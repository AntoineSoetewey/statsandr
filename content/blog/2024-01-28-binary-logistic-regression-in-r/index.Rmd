...
description: Learn when and how to use a (univariate and multivariate) binary logistic regression in R. Learn also how to interpret, visualize and report results
output:
  blogdown::html_page:
    toc: true
    toc_depth: 6  
...
Logistic regressions and poisson regressions are both part of a broader type of model called **generalized linear models** (abbreviated as **GLM**). The name "generalized linear models" comes from the fact that these models allow to "generalize" the classic linear model. Indeed, it can be used in many situations, for example when analyzing a dependent variable which is not necessarily quantitative continuous or normally distributed (which are prerequisites for a linear model).
...
In our case, `heart_rate` (our dependent variable) is currently encoded as [integer](/blog/data-types-in-r/#integer) with values ranging from 0 to 4. Therefore, we first classify it into 2 classes by setting 0 for 0 values and 1 for non-0 values, using the `ifelse()` function:
...
Therefore, the probability of developing a heart disease increases by (`r round(exp(coef(m1)[2]), 3)` - 1) $\times$ 100 = `r (round(exp(coef(m1)[2]), 3) - 1) * 100`% when a patient becomes one year older.
...
- Moreover, when looking at the coefficient for sex, $\hat{\beta}_1 =$ `r round(coef(m2)[2], 3)`, we can say that:
  ...
  + This means that, the probability of developing a heart disease are (`r round(exp(coef(m2)[2]), 3)` - 1) $\times$ 100 = `r (round(exp(coef(m2)[2]), 3) - 1) * 100`% higher for males than for females.
...
The avid reader will notice that a univariate binary logistic regression with a qualitative independent variable will lead to the same conclusion than a [Chi-square test of independence](/blog/chi-square-test-of-independence-in-r/):
...
The main advantages of using a multivariate logistic regression compared to a univariate logistic regression are to:
...
- The last three columns give the OR, and in parentheses the 95% CI of the OR and the $p$-value (for the univariate and the two multivariate models, respectively).
...
I now hope that (univariate and multivariate) binary logistic regressions in R no longer hold any secrets for you.
