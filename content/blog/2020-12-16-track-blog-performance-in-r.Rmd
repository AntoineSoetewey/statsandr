...
description: Learn how to track the performance of your blog or website in R by analyzing page views, sessions, users and engagement with the {googleAnalyticsR} package
output:
  blogdown::html_page:
    toc: true
    toc_depth: 6
...
In this article, I show how to **analyze a blog and its blog posts** with the `{googleAnalyticsR}` R package (see package's [full documentation](https://8-bit-sheep.com/googleAnalyticsR/){target="_blank"}). After sharing some analytics about the blog, I will also discuss about content creation/distribution and, to a smaller extent, the future plans. This is a way to share my journey as a data science blogger and a way to give you an insight about how Stats and R is doing.
...
I am not an expert in the field of digital marketing, but who knows, it may still give some ideas to data analysts, SEO specialists or other bloggers on how to track the performance of their own blog or website using R. For those of you who are interested in a more condensed analysis, see my [custom Google Analytics dashboard](https://antoinesoetewey.com/files/google-analytics-dashboard).^[Thanks to the [RStudio blog](https://blog.rstudio.com/2021/01/06/google-analytics-part2/){target="_blank"} for the inspiration.]
...
Make sure to run the `ga_auth()` function in a R script and not in a [R Markdown](/blog/getting-started-in-r-markdown/) document. Follow this [procedure](https://8-bit-sheep.com/googleAnalyticsR/articles/rmarkdown.html){target="_blank"} if you want to use the package and its functions in a R Markdown report or in a blog post like I did for this article.
...
As you can see I have two accounts linked to my Google Analytics profile: one for my personal website ([antoinesoetewey.com](https://antoinesoetewey.com/){target="_blank"}) and one for this blog.
...
In its first year, Stats and R has attracted **`r format(gadata$users, big.mark = ",")` users**, who generated a total of **`r format(gadata$sessions, big.mark = ",")` sessions** and **`r format(gadata$pageviews, big.mark = ",")` page views** (that is an average of `r pander(gadata$pageviews / (as.numeric(end_date - start_date) + 1))` page views per day).
...
Note that I decided to focus on the number of sessions and the number of page views in this section and the following ones, but you can always change to your preferred metrics by editing `metrics = c("sessions")` in the code. See all available metrics provided by Google Analytics in this [article](https://ga-dev-tools.appspot.com/dimensions-metrics-explorer/){target="_blank"}.
...
The difference in sessions between weekdays is, however, not as large as expected.
...
Most visits were, as expected, from `r pander(top_n(browser, n = 3, n)$Browser)` browsers.
...
* some readers on tablet have a very low average time spent on each page (see the peak around 0 second in the tablet facet)
* distributions of the average time on page for readers on desktop and mobile were quite similar, with an average time on page mostly between 100 seconds (= 1 minute 40 seconds) and 400 seconds (= 6 minutes 40 seconds)
