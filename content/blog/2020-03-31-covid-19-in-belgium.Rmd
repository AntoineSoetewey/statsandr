...
Next, we need to create a vector with the daily cumulative incidence for Belgium, from February 29 (when our daily incidence data starts), through to March 30 (last available date at the time of publication of this article). We will then compare the predicted incidence from the *SIR* model fitted to these data with the actual incidence since February 29. We also need to initialise the values for *N*, *S*, *I* and *R*. Note that the daily cumulative incidence for Belgium is extracted from the [`{coronavirus}` R package](/blog/top-r-resources-on-covid-19-coronavirus/#coronavirus) developed by Rami Krispin.
...
# Feb 29 to March 30 into a vector called Infected
library(lubridate)
...
From the above graph we see that there were more observed confirmed cases than expected until March 28, a simliar amount on March 29 and then a lower number of observed confirmed cases than expected on March 30. More data would be needed to see whether this downard trend is confirmed in the long term.
...
nand the rate of incidence seems to decrease for a week (perhaps thanks to the new containment measures?).
