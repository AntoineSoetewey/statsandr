...
Integer data type is actually a special case of numeric data. Integers are numeric data without decimals. It can be used if you are sure that the numbers you store will never contains decimals. For example, let's say you are interested in the number of children in a sample of 10 families. This variable is a discrete variable (see a reminder on the [variable types](/blog/variable-types-and-examples/) if needed) and will never have decimals. Therefore, it can be stored as integer data thanks to the `as.integer()` command:
...
Furthermore, as soon as there is at lease one character value inside a variable or vector, the whole variable or vector will have the character format:
...
As you can see from the results above, a space within character data makes it a different string in R!
