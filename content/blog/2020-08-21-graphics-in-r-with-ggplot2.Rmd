...
In the context of simple [linear regression](/blog/multiple-linear-regression-made-simple/), it is often the case that the regression line is displayed on the plot. This can be done by adding `method = lm` (`lm` stands for linear model) in the `geom_smooth()` layer:
