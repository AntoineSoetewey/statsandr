...
Depending on your data, it is possible to format axes in a certain way with the `{scales}` package. The formats I use the most are `comma` and `label_number_si()` which formats large numbers in a more-readable way.

For this example, we multiply both variables by 1000 to have larger numbers and then we apply a different format for each axis:
...
These two formats make large numbers easier to read. Other formats are possible such as using dollar signs, percentage signs, dates etc. See more information in the [package's documentation](https://scales.r-lib.org/){target="_blank"}.
