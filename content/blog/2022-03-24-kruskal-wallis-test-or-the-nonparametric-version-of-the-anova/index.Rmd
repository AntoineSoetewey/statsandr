...
tags:
  ...
  - R
  - Statistics
meta_img: images/kruskal-wallis-test-nonparametric-version-anova.jpeg
...
The Dunn test and pairwise Wilcoxon test being the most common ones among the four, here is how to do them in R.^[Note that there are other *p*-value adjustment methods. See `?dunnTest` and `?pairwise.wilcox.test` for more options.]
...
Here again, we conclude that all 3 species are significantly different in terms of flipper length (*p*-values < 0.001).
...
We also showed the most common post-hoc tests after a significant Kruskal-Wallis test---the Dunn and pairwise Wilcoxon tests.
