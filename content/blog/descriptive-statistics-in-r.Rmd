...
I illustrate each of the 4 functions in the following sections. See the setup settings in the [vignette](https://cran.r-project.org/web/packages/summarytools/vignettes/Introduction.html){target="_blank"} of the package if you want to print the outputs in a nice way in R Markdown.^[Note that the `plain.ascii` and `style` arguments are needed for this package. In our examples, these arguments are added at the chunk-level so they are not visible.]
...
freq(dat$Species, report.nas = FALSE)
```

And for a minimalist output with only counts and proportions:

```{r, plain.ascii = FALSE, style = "rmarkdown"}
freq(dat$Species, report.nas = FALSE, totals = FALSE, 
