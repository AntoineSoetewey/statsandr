...
I illustrate each of the 4 functions in the following sections. See the setup settings in the [vignette](https://cran.r-project.org/web/packages/summarytools/vignettes/Introduction.html){target="_blank"} of the package if you want to print the outputs in a nice way in R Markdown.^[Note that the `plain.ascii` and `style` arguments are needed for this package. In our examples, these arguments are added in the settings of each chunk so they are not visible.]
...
This is equivalent than `table(dat$Species, dat$size)` performed in an earlier [section](/blog/descriptive-statistics-in-r/#contingency-table).
