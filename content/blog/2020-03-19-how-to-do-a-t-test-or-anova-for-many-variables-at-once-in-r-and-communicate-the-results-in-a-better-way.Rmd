...
- **Address the issue of multiple testing**: with MANOVA, the error rate equals the significance level (with no *p*-value adjustment method needed).
...
* If you want to report statistical results on a graph, I advise you to check the [`{ggstatsplot}` package](https://indrajeetpatil.github.io/ggstatsplot/){target="_blank"} and in particular the `ggbetweenstats()` and `ggwithinstats()` functions. These functions allow to compare a continuous variable across multiple groups or conditions (for both independent and paired samples). Two advantages of the functions is that:
  + it is very easy to switch from parametric to nonparametric tests and
...
We illustrate the routine for two groups with the variables `sex` (two factors) as independent variable, and the 4 quantitative continuous variables `bill_length_mm`, `bill_depth_mm`, `flipper_length_mm` and `body_mass_g` as dependent variables:
