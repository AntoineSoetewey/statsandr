...
These questions led me to the idea of creating a simple (yet powerful and visually appealing) dashboard, as opposed to a [Shiny app](/tags/shiny). The advantage of a dashboard over a Shiny app is that it is much easier to code (especially if you are proficient in R Markdown). On the other hand, Shiny apps are interactive in the sense that users can edit the outputs and the visualizations by simply changing some inputs in a user-friendly way, while dashboards are static and cannot be modified by the final user.

# Coronavirus dashboard: the case of Belgium

Amongst all the visualizations I have seen so far, one was standing out by its simplicity and at the same time, by its completeness and by the quality of its visualizations. I thus decided to create a Coronavirus dashboard based on this already existing [dashboard](https://ramikrispin.github.io/coronavirus_dashboard/){target="_blank"} by Rami Krispin (which comes with a license that allows to be freely adapted and shared) and adapt it so that it is specific to Belgium. Note that I also removed some visualizations and tables from the initial dashboard to keep it really simple and straight to the point.

Before reading further, here is my [Coronavirus dasboard](https://www.antoinesoetewey.com/files/coronavirus-dashboard.html){target="_blank"} adapted to Belgium and previews of the main sections below:

![](/blog/2020-03-23-how-to-create-a-simple-coronavirus-dashboard-specific-to-your-country-in-r_files/coronavirus-dashboard-the-case-of-belgium.png){width=100%}

![](/blog/2020-03-23-how-to-create-a-simple-coronavirus-dashboard-specific-to-your-country-in-r_files/coronavirus-dashboard-the-case-of-belgium-2.png){width=100%}

![](/blog/2020-03-23-how-to-create-a-simple-coronavirus-dashboard-specific-to-your-country-in-r_files/coronavirus-dashboard-the-case-of-belgium-3.png){width=100%}
...
* The map section shows a world map of the confirmed cases, recovered cases and deaths. You can uncheck one or several types of cases (top right corner) and zoom in or out (top left corner).
* The about section gives more information about the data, how often it is updated and the dashboard in general.

I believe this simple dashboard is easy enough to be adapted to any country (and by anyone from beginner to expert), and still communicate key measures about the virus throughout some visualizations. A little extra which is worth mentioning is the fact that all plots are generated with the `{plotly}` package. This package allows to enhance plots by displaying relevant information when hovering over them (try by yourself!). 
...
1. Click on the button "Source code" located in the top right corner of the dashboard. Copy that code.
...
6. In the code, replace `Belgium` with your country. Change also the title and the author at the top of the document, and edit the about section at the bottom of the document.
6. Knit the document (see this [article](/blog/getting-started-in-r-markdown) if you are unfamiliar with R Markdown). Your dashboard should appear in HTML format.

Following these 7 steps, you should already have a simple dashboard specific to your country. I have intentionnally kept it simple so that everyone could copy it and have their own dashboard in a limited amount of time.
