...
Given that the project requires the use of simulations, we focused on answering this question via simulations in R. That being said, we also verify results obtained through simulations with results obtained using probability theory.
...
This will be done through simulations, but for the sake of completeness we will also verify our results with 
...
For the sake of completeness, we now compare results obtained through simulations with results obtained from [probability theory](/blog/the-9-concepts-and-formulas-in-probability-that-every-data-scientist-should-know/).
...
```{r}
# number of possible combinations of two-letter initials
n_initials <- 26^2
...
The probability that at least two persons have the same initials in a team of `r n_persons` is `r round(have_same(n_initials, n_persons) * 100, 2)`%. This is relatively close to the probability found with simulations.
...
Finally, we visualize the probabilities:
...
- All letters of the alphabet had the same probability of occurring, meaning that all pairs of initials were equally probable (which is probably not the case in reality). This bias could be limited by specifying different weights when sampling initials.
