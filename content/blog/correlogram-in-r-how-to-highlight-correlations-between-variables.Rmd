...
The correlogram represents the correlations for all pairs of variables. Positive correlations are displayed in blue and negative correlations in red. The intensity of the color is proportional to the correlation coefficient so the stronger the correlation, the darker the boxes. The color legend on the right hand side of the correlogram shows the correlation coefficients and the corresponding colors.
...
Finally, the white boxes indicate that the correlation is not significantly different from 0 at the specified significance level (in this example, at $\alpha = 5$%), which means that there is no **linear** relationship between the two variables considered.^[A correlation test is performed in order to test whether the correlation is significantly different from 0. Remind that the null and alternative hypotheses of this test are: $H_0$: $\rho = 0$ and $H_1$: $\rho \ne 0$, where $\rho$ is the correlation coefficient.]
