...
There are three ways to assign an object in R: `<-`, `=` or `assign()`:
...
You can also assign a vector to another vector:
...
We can also use [booleans](/blog/data-types-in-r/#logical) (i.e., `TRUE` or `FALSE`) to select some elements of a vector:
...
We can find the type of a vector with the `is.type` functions:
...
As you can see, the 4 first elements of the vector are conserved while all others are removed.

## Numerical operators

Several numerical operators such as `+`, `-`, `*`, `/` and `^` can be applied to vectors:
...
The following operations can be applied too: `sqrt` (square root), `cos`, `sin`, `tan`, `log`, `log10`, `exp`, `abs` (absolute value):
...
To extract a character string from the beginning and the end positions, we can use the `substr()` function:
...
Split a character string with the `strsplit()` function:
...
As you can see, the third element of the vector is the smallest and the second element is the largest.
...
We create factors with the `factor()` function:
...
Remember that the gold standard for a data frame is that columns represent variables, lines correspond to observations and each value must have its own cell:
...
In this article, we use the data frame `cars` to illustrate the different data manipulation techniques. Note that the data frame is installed by default in RStudio (so you do not need to import it) and I use the generic name `dat` as the name of the data frame throughout the article (see [here](/blog/how-to-import-an-excel-file-in-rstudio/#user-friendly-way) why I always use a generic name instead of more specific names). Here is a table of the whole data frame:
...
This data frame has `r nrow(dat)` observations with `r ncol(dat)` variables (`speed` and `distance`). You can check the number of observations and variables with `nrow(dat)` and `ncol(dat)`, or `dim(dat)`:
...
As you probably figured out by now, you can select observations and/or variables of a dataset by running `dataset_name[row_number, column_number]`. When the row or column number is left empty, the entire row/column is selected. Note that all examples presented above also work for matrices:
...
The above code will split your data frame into several data frames, one for each level of the factor variable.
...
Note than in programming, a character string is generally surrounded by quotes (`"character string"`) and R is not an exception.
...
rbind(dat1, dat3) # r stands for row
```
...
dat2
dat3
cbind(dat2, dat3) # c stands for column
```

# Missing values

Missing values (represented by NA in RStudio, for "Not Applicable") are often problematic for many analyses because every computation including a missing value has a missing value for result.

For instance, the mean of a series or variable with at least one NA will give a NA (the data frame `dat` created in the previous section is used for this example):
...
```{r}
anyNA(dat$variable2) # check for NA in variable1
...
We can extract weekdays, months, quarters and years as follows:
...
or using `write.table()` or `write.csv()`:
