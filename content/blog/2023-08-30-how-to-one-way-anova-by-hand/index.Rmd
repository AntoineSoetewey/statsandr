...
An ANOVA is a statistical test used to compare a [quantitative variable](/blog/variable-types-and-examples/#quantitative) between groups. In theory, it can be done with two groups.^[In that case, a [Student's t-test](/blog/student-s-t-test-in-r-and-by-hand-how-to-compare-two-groups-under-different-scenarios/) is usually preferred over an ANOVA, although both tests will lead to the exact same conclusions.] However, in practice, it is used to compare three or more groups.
...
To illustrate the method, suppose we take a sample of 12 students, divided equally into three classes (A, B and C) and we observe their age. Here is the [sample](/blog/what-is-the-difference-between-population-and-sample/):
...
Remember that the null hypothesis of the ANOVA is that all means are equal, whereas the alternative hypothesis is that at least one mean is different from the other two. Formally, we have:
...
$$\frac{24 + 31 + 26 + 23 + 24 + 21 + 19 + 24 + 15 + 21 + 18 + 18}{12} = 22$$
