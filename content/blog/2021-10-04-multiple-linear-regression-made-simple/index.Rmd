...
Remember that [descriptive statistics](/blog/descriptive-statistics-in-r/) is a branch of statistics that allows to describe your data at hand.

Inferential statistics (with the popular [hypothesis tests](/blog/hypothesis-test-by-hand/) and confidence intervals) is another branch of statistics that allows to make inferences, that is, to draw conclusions about a population based on a [sample](/blog/what-is-the-difference-between-population-and-sample/).
...
This is the reason you will often read "[Correlation does not imply causation](/blog/correlation-coefficient-and-correlation-test-in-r/#correlation-does-not-imply-causation)" and linear regression follows the same principle.
...
![Source: James, G., Witten, D., Hastie, T., & Tibshirani, R. (2013)](images/multiple-linear-regression-plane.png){width=100%}
...
- the test of $\beta_3 = 0$ corresponds to testing the hypothesis: is fuel consumption associated with displacement, at a constant level of weight and horsepower
...
- Multicollinearity (bottom left plot) is not an issue (I tend to use the threshold of 10 for VIF, and all of them are below 10)^[I use the threshold of 10 because, as shown by @james2013introduction, a value between 5 and 10 indicates a moderate correlation, while VIF values greater than 10 indicate a high and *non-tolerable* correlation.]
- There is no influential points (middle right plot)
- Normality of the residuals (bottom right plot) is also not perfect due to 3 points deviating from the reference line but it still seems acceptable to me. In any case, the number of observations is large enough given the number of parameters^[@austin2015number showed that two subjects per variable tends to permit accurate estimation of regression coefficients in a linear regression model estimated using ordinary least squares. Moreover, the general rule of thumb says that there should be at least 10 observations per variable [@schmidt2018linear]. Our dataset contains 32 observations, above the minimum of 10 subjects per variable.] and given the small deviation from normality so tests on the coefficients are (approximately) valid whether the error follows a normal distribution or not
...
To improve linearity, the variable could be removed, a transformation could be applied (logarithmic and/or squared for instance) or a quadratic or cubic (or even a higher-order polynomial) term could be added to the model.^[If you apply a logarithmic transformation, see two guides on how to interpret the results: in [English](/blog/multiple-linear-regression-made-simple/images/Interpret-Regression-Coefficient-Estimates-in-linear-regression.png) and in [French](https://www.parisschoolofeconomics.eu/docs/yin-remi/interpretation-des-coefficients.pdf){target="_blank"}.] If this does not fix the issue of linearity, other types of models could be considered.

If you want to read more about these conditions of applications and how to deal with them, here is a very complete [chapter](http://quantpsych.net/stats_modeling/diagnostics.html) on diagnostics for linear models written by Prof. Dustin Fife.
...
Note that in a simple linear regression model, the coefficient of determination is equal to the square of the Pearson [correlation coefficient](/blog/correlation-coefficient-and-correlation-test-in-r/):
...
Note that if you want to compare models with different number of independent variables, it is best to refer to the adjusted $R^2$ (= `r round(summary(model2)$adj.r.squared, 4)` here).
...
I hope this article helped you to understand better linear regression and gave you the confidence to do your own linear regressions in R. If you need to model a binary variable instead of a quantitative continuous variable, see how to perform a [binary logistic regression in R](/blog/binary-logistic-regression-in-r/).
